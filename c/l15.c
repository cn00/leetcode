...
int** threeSum99(int* nums, int numsSize, int* returnSize, int** returnColumnSizes){
    *returnSize = 0;
    if (numsSize == 0) {
        return 0;
    }
...
void printArrys(int** p, int sizep){
...
int** threeSum(int* nums, int numsSize, int* returnSize, int** returnColumnSizes){
    ...
    qsort(nums, numsSize, sizeof(int), comp);
    // printArry(nums, numsSize);

    int** rp = (int **)malloc(sizeof(int *) * numsSize*6);
    *returnColumnSizes = (int*)malloc(sizeof(int) * numsSize*6);
    int i = 0, j = numsSize - 1, k = -1;
    int t = 0;
    int c = 0;
    for(i = 0; i < numsSize&&nums[i]<0;++i){
        for(j = numsSize - 1; j > i+1&&nums[j]>0 ; --j){
            t = nums[i] + nums[j];
            k = binsearch(nums, -t, i+1, j-1);
            // printf("[%d]:%d,[%d]%d %d->%d\n", i, nums[i], j, nums[j], -t, k);
            if(k>0){
                rp[c] = (int*)malloc(3*sizeof(int));
                rp[c][0] = nums[i];
                rp[c][1] = nums[j];
                rp[c][2] = nums[k];
                (*returnColumnSizes)[c] = 3;
                ++c;
...
int main(int c, char** vv){
    ...
    int** rr = threeSum(s, sizeof(s)/sizeof(int), &rs, &rcs);
    // printArry(s, sizeof(s)/sizeof(int));
    printf("\n");
    printArrys(rr, rs);
